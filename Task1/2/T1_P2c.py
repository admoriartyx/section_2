# Okay it looks like all that part c asks is to repeat a and b but with the -5 to +5 boundary expansion.
# I will just adjust my code a little bit and rerun the old programs.

# Since I used np.random.rand(n, 2) before, I consulted chatGPT as to if there is 
# a quick way to amend this function to allow for values between -5 and 5.
# The AI's suggestion was clever. It advised to take every value generated by the numpy function
# and to multiply by 10, then subtract 5. This would rescale the bounds of the points, which I think is cool,
# So I will redefine the original point statements and use the rest of the code as written.

import numpy as np
import matplotlib.pyplot as plt
import time

points_10 = np.random.rand(10, 2) * 10 -5
points_50 = np.random.rand(50, 2) * 10 -5
points_100 = np.random.rand(100, 2) * 10 -5
points_200 = np.random.rand(200, 2) * 10 -5
points_400 = np.random.rand(400, 2 )* 10 -5
points_800 = np.random.rand(800, 2) * 10 -5
points_1000 = np.random.rand(1000, 2) * 10 -5
...
plt.title("Convex Hull Runtimes -5 to 5")
plt.xlabel("n Point Cloud")
plt.ylabel("Runtimes")
plt.legend()
plt.grid(True)
plt.show()
plt.savefig("convex_hull_runtimes_new_range.png")

# Success again. Also I think I mentioned it prior but the plotted outputs of my code
# can only be viewed by being saved with the plt.savefig function. They should be uploaded
# to Github when I commit (hopefully). Just wanted to throw that in there, not sure what
# you are seeing on your side.